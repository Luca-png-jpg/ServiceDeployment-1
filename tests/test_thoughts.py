...
@pytest.fixture
def client():
    config = {"TESTING": True, "SQLALCHEMY_DATABASE_URI":"sqlite:///:memory:", "SQLALCHEMY_TRACK_MODIFICATIONS": False}
...
def test_post_thought_valid_token(client):
    data = {"username": "mario", "text": "Ciao a tutti"}
    ...
    assert json_data["text"] == "Ciao a tutti"
...
def test_get_thought_by_id(client):
    with client.application.app_context():
        t = ThoughtModel(username="anna", text="Un pensiero")
        db.session.add(t)
        db.session.commit()
        tid = t.id
